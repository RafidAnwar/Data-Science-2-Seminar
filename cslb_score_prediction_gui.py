...
def CSLB_SCORE_PREDICTION_ANALYSIS():
    ...
    def process_data():
        ...
        try:
            ...
            start_loading_animation() # Start the result is loading animation.

            # Run a subprocess to execute the prediction model script with the collected data.
            ...
            stop_loading_animation() # Stop the result is loading animation.
    ...
    # functions to make the loading animation
    ...
    # Update the result label with the prediction result.
    ...
    def submit_follow_up_data(follow_up_window):
        ...
        try:
            follow_up_data = (
                ...
                frequency_6mo_map[cslb_pace_6mo_ip.get()],
                frequency_6mo_map[cslb_stare_6mo_ip.get()],
                frequency_6mo_map[cslb_defecate_6mo_ip.get()],
                frequency_6mo_map[cslb_food_6mo_ip.get()],
                frequency_6mo_map[cslb_recognize_6mo_ip.get()],
                activity_6mo_map[cslb_active_6mo_ip.get()]
            )

            # Execute the Python script for follow-up data analysis using subprocess.
            # Pass the collected follow-up data to the script.
    ...
    def show_follow_up_window():
        # Display the follow-up window if certain conditions in the prediction result are met.
        # Contain additional input fields for follow-up data.
        follow_up_window = ctk.CTkToplevel(app)
        follow_up_window.title("6-Month Follow-Up Questions")
        follow_up_window.geometry("+300+800")
        follow_up_window.attributes('-topmost', True)
    ...
    # Layout the initial input fields
    input_fields = [
    ...
    frame_dog_age.grid(row=0, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_dog_weight.grid(row=1, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    #dog_weight_ip.set("Please select an option")
    ...
    frame_dog_activity.grid(row=2, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_eye_health.grid(row=3, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_ear_health.grid(row=4, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_pacing.grid(row=5, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_staring.grid(row=6, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_stuck.grid(row=7, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_recognition.grid(row=8, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_walk_walls.grid(row=9, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_avoid.grid(row=10, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    frame_find_food.grid(row=11, column=0, columnspan=2, padx=10, pady=2, sticky="ew")
    ...
    loading_label.grid(row=13, column=0, columnspan=2)
